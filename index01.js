...
app.get('/user/:username',(req,res)=>{
    ...
    <div class="container shadow" style="margin: 5% auto; padding: 3% 2%">
        ...
        <div class="row">
        <button type="button" class="btn btn-success col-2" onclick="window.location.pathname = window.location.pathname + '/add'"><i class="fa fa-plus-circle"> Add new product</i></button>
        <span class="col-8"></span>
    ...
    `);
        con.query(`SELECT * FROM products`,(err,result)=>{
            if(err) throw err;
            var sort_num = 5;
            var page_num = Math.floor(result.length/sort_num);
            var qoldiq = result.length%sort_num;
            var item_now = 0;
            for(let k=0; k<=page_num; k++){
                if(k<page_num){
                    res.write(`<tr><td>Page: ${k+1}</td></tr>`);
                    for(let i=0; i<sort_num;i++){
                        res.write(`
                            <tr class="row">
                                <td class="col-2 text-center">${result[item_now].id}</td>
                                <td class="col-4 text-center">${result[item_now].name}</td>
                    ...
                    res.write('<tr><td>PAGE END</td></tr>')
                    for(let i=0; i<qoldiq;i++){
                        res.write(`
                            <tr class="row">
                                <td class="col-2 text-center">${result[item_now].id}</td>
                                <td class="col-4 text-center">${result[item_now].name}</td>
